...
- **Registro por texto 100% quebrado em produção com "Serviço de IA temporariamente indisponível"** — a Groq aposentou `llama-3.3-70b-versatile` (a família Llama saiu do catálogo de chat) e passou a responder 404 `model_not_found`. Texto migrado para `openai/gpt-oss-120b`, escolhido por expor o raciocínio em campo `reasoning` separado e devolver `content` com JSON limpo — o `qwen` de visão, testado como alternativa, vaza `<think>` no conteúdo e quebra o parse. Segunda causa somada à primeira: **`GROQ_MAX_TOKENS=8192` também já estourava sozinho** o teto de 8000 tokens por minuto do free tier, porque a Groq *reserva* `max_tokens` na conta do limite (799 de prompt + 8192 = 8991 pedidos contra teto de 8000 → 413 Payload Too Large). Reduzido para 2048, com folga de ~5900 para o prompt contra a maior entrada medida (visão, 3204 tokens, constante). Trocar só o nome do modelo não teria resolvido.
- **Descontinuação de modelo de texto chegava ao log como um 404 cru** — o caminho de visão já traduzia `NotFoundError` numa mensagem que nomeia o modelo e a variável a ajustar; o de texto não, e o diagnóstico virou garimpo em log do `httpx`. `_call` agora falha rápido dizendo qual modelo não existe mais.
- **Tabela `portions` vazia em produção fazia o registro responder 200 com calorias erradas** — `seed_portions.py` nunca constou do procedimento de deploy, então toda unidade caseira caía em `portion_source: "sem_ancora"`: "2 fatias de pizza de calabresa" devolvia **5,4 kcal** em vez de 540. Tabela populada (125 linhas) e o seed passou a ser passo obrigatório e verificável no checklist de `docs/deploy.md` — a falha é silenciosa por natureza, já que o item só vem marcado com `needs_review` depois de o número aparecer para o usuário.
- **Nomes de modelo fixos em scripts apodreciam junto com o catálogo da Groq** — `scripts/translate_foods.py` e `tests/smoke_test.py` tinham o modelo travado no código; o smoke test ainda apontava para o modelo de visão aposentado em 2026-07, e reportava falso negativo desde então. Ambos passam a ler de `GROQ_TEXT_MODEL`/`GROQ_VISION_MODEL`.
