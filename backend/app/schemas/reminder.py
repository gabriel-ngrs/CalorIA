...
class ReminderCreate(BaseModel):
    type: ReminderType
    time: _time
    days_of_week: list[int | str] = Field(default=[0, 1, 2, 3, 4, 5, 6])
    message: str | None = None

    @field_validator("days_of_week", mode="before")
    @classmethod
    def validate_days(cls, v: list[int | str]) -> list[int]:
        if not v:
...
class ReminderUpdate(BaseModel):
    ...
    @field_validator("days_of_week", mode="before")
    @classmethod
    def validate_days(cls, v: list[int | str] | None) -> list[int] | None:
        if v is None:
            return v
        if not v:
            raise ValueError("days_of_week não pode ser vazio")
