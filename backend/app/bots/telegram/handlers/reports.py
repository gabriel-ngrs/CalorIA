...
async def lembrete_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_html(
            "❌ Hora inválida. Use o formato HH:MM (ex: 07:30)"
        )
        return
...
async def lembretes_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for r in reminders:
        ...
        lines.append(
            f"{status} <code>#{r.id}</code> {r.type.value} — {r.time.strftime('%H:%M')} ({days})"
        )

    await update.message.reply_html("⏰ <b>Seus lembretes</b>\n\n" + "\n".join(lines))


async def remover_lembrete_command(
    ...
    if deleted:
        await update.message.reply_html(
            f"✅ Lembrete <code>#{reminder_id}</code> removido."
        )
    else:
        await update.message.reply_html(
            f"❌ Lembrete <code>#{reminder_id}</code> não encontrado."
        )
