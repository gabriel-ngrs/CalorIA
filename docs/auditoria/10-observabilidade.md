...
- AUD-052 — Caddy não configura `X-Forwarded-*` explícito, não tem rate limit, log em texto humano; ausência de headers de segurança já mapeada em AUD-041 (🟢 baixa; combina com AUD-040/041).
- AUD-053 — Dois deployments concorrentes commitados (`Caddyfile` + `docker-compose.yml` full-stack vs `Caddyfile.backend` + `docker-compose.backend.yml` backend-only) — CD usa o backend-only, mas full-stack continua no repo sem documentação clara (🟢 baixa; decisão pendente do mantenedor).
...
**Dois Caddyfiles commitados:**

| Arquivo | Linhas | Cenário | Uso atual (CD) |
|---|---|---|---|
| `Caddyfile` | 56 | full-stack (frontend Next + backend FastAPI + docs) | **não referenciado** pelo `cd.yml` |
| `Caddyfile.backend` | 22 | backend-only (apenas API + docs + health) | **ativo** — montado em `docker-compose.backend.yml:75` que é usado em `.github/workflows/cd.yml:33` |

**O que o `Caddyfile` (full-stack) faz:**

1. `handle /api/auth/*` → `frontend:3000` (NextAuth precisa ficar no Next, não no backend)
2. `handle /api/*` → `backend:8000`
3. `handle /docs*`, `/redoc*`, `/openapi.json`, `/health` → `backend:8000`
4. `handle` (catchall) → `frontend:3000`
5. `log { output stdout, format console, level INFO }`

**O que o `Caddyfile.backend` (deploy ativo) faz:**

1. `handle /api/*` → `backend:8000` (sem regra `/api/auth/*` separada — frontend não está no Caddy)
2. `handle /docs*`, `/redoc*`, `/openapi.json`, `/health` → `backend:8000`
3. Mesma config de log.
4. **Sem catchall** — sem frontend para servir.

**Conclusão sobre a "decisão pendente" (`runbook 11.4`):** o frontend produção provavelmente está hospedado fora (Vercel preview confirmado em AUD-044 — `frontend-nine-mu-59.vercel.app`). O `Caddyfile.backend` + `docker-compose.backend.yml` são os artefatos **vivos**. O `Caddyfile` + `docker-compose.yml` foram introduzidos antes (full-stack self-hosted) e nunca foram removidos quando a decisão "frontend na Vercel" foi tomada. Esse é o conteúdo do AUD-053 — registro, sem prescrição.

**Checklist do runbook § J.5:**

| Item | Estado | Notas |
|---|---|---|
| HTTPS automático | ✅ | `{$APP_DOMAIN}` é envelope padrão do Caddy + Let's Encrypt automático |
| Logs estruturados | ❌ | `format console` é texto humano, não JSON. Trocar por `format json` resolve — comparado a AUD-049 (logs do backend), ainda mais barato porque Caddy tem `format json` builtin |
| `X-Forwarded-*` | ⚠️ implícito | Caddy v2 já injeta `X-Forwarded-For` e `X-Forwarded-Proto` automaticamente no `reverse_proxy` — confiável, mas vale documentar |
| Rate limiting | ❌ | Sem `caddy-ratelimit` ou diretiva equivalente — combina com AUD-040 (sem rate limit no backend) |
| Headers de segurança | ❌ | AUD-041 já cobre — sem bloco `header { }` no Caddy nem em outras camadas |

**Recomendação consolidada (AUD-052)** — Esforço S (< 1h):

1. **Logs em JSON**: trocar `format console` por `format json` em ambos Caddyfiles. Saída estruturada com `ts`, `request.host`, `request.uri`, `request.method`, `status`, `duration`, `bytes_read`, `bytes_written` — facilita rotação para Loki/CloudWatch.
2. **Bloco `header { }` para segurança** (cobre AUD-041 inteiro): X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, CSP. Cuidado: aplicar no `Caddyfile.backend` (deploy ativo) também.
3. **Rate limit no edge** via `caddy-ratelimit` plugin: defesa em profundidade ao backend (combina com AUD-040). 5 req/s/IP para `/api/auth/*` chega a bloquear credential stuffing antes de tocar FastAPI.
4. **Documentar a decisão sobre os 2 Caddyfiles** (cf. AUD-053): manter só `Caddyfile.backend` se o frontend vai ficar na Vercel; OU manter os 2 com README explicando "este é o full-stack, este é o backend-only" — hoje ambiguidade leva a edits no arquivo errado.
