...
- **Descrição:** O teste e2e "deve fazer login com usuário existente" preenche o formulário com:
    - email: `email-redigido@example.com` (Gmail real do mantenedor — verificado via `git log --format="%ae"` no histórico de commits)
    - senha: `SENHA-REDIGIDA` (senha real, em texto claro)
  ...
  **Vetor de risco:**
    ...
    2. **Password reuse** — se `SENHA-REDIGIDA` for usada em outros serviços (Google, banco, redes sociais), o blast radius é massivo. É a possibilidade mais provável de explorar uma vez vazada.
    3. Se a conta `email-redigido@example.com` for o login do Google de recuperação de outras contas, o atacante com a senha pode tentar credential stuffing global.
...
- **Recomendação:** **Plano em 4 etapas, ordem importa**:
    1. **AGORA — trocar a senha** em `email-redigido@example.com` (no app CalorIA) **e** em qualquer outro serviço onde tenha sido reusada. Esta etapa não pode esperar PR review.
