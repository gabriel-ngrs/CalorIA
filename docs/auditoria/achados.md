...
Lista de problemas encontrados, ordenada por severidade (🔴 → 🟠 → 🟡 → 🟢) e, dentro de cada severidade, por frente (A → K) e ID. Para análise priorizada ver `relatorio-preliminar.md`.

**Status totais:** críticos: 2 · altos: 14 · médios: 21 · baixos: 20 · **total: 57**
...
- **Descrição:** O teste e2e "deve fazer login com usuário existente" preenche o formulário com:
    - email: `<e-mail pessoal do mantenedor>` (Gmail real do mantenedor — verificado via `git log --format="%ae"` no histórico de commits)
    - senha: `[REDIGIDO]` (senha real, em texto claro)

  Os outros testes do mesmo arquivo (linhas 27-29) usam constantes `TEST_EMAIL`/`TEST_PASSWORD` (geradas, descartáveis); este teste específico foi escrito com a credencial direto no source. O par foi commitado em **`4737257` (2026-04-29)** e o repo está hospedado em `https://github.com/gabriel-ngrs/CalorIA.git` — qualquer pessoa com acesso de leitura ao repo (ou ao histórico, mesmo após remoção do HEAD) consegue extrair as credenciais a partir do histórico do git (comando de extração omitido deste documento por segurança).

  **Vetor de risco:**
    1. Acesso à conta CalorIA do mantenedor (refeições, peso, conversas IA — dados sensíveis de saúde).
    2. **Password reuse** — se `[REDIGIDO]` for usada em outros serviços (Google, banco, redes sociais), o blast radius é massivo. É a possibilidade mais provável de explorar uma vez vazada.
    3. Se a conta `<e-mail pessoal do mantenedor>` for o login do Google de recuperação de outras contas, o atacante com a senha pode tentar credential stuffing global.
...
- **Recomendação:** **Plano em 4 etapas, ordem importa**:
    1. **AGORA — trocar a senha** em `<e-mail pessoal do mantenedor>` (no app CalorIA) **e** em qualquer outro serviço onde tenha sido reusada. Esta etapa não pode esperar PR review.
