...
**Cross-referências**:
- AUD-038 (credenciais hardcoded em `e2e/auth.spec.ts`) **seria pego** por `gitleaks` rodando como pre-commit hook. A senha `SENHA-REDIGIDA` é detectável pela regex de Generic API Key / High Entropy String que vem no `gitleaks` default config.
